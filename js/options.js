...
function init_listeners() {
  element.context.addEventListener('click',
                                   function() // show that restart is requred
                                   {
                                     toggle("contx_info");
                                     save_options();
                                   });

  element.test.addEventListener(
      'click', function() // test listener
      {
        set_vars(vars);
  ...
  element.hotkeys.addEventListener("keydown", function(e) {
    keyDown(e, 0);
    save_options();
  }, false); // keyboard shortcuts

  element.volume.addEventListener('change',
                                  function() // display volume level
                                  {
                                    element.percents.innerHTML =
                                        parseInt(this.value) + ' %';
                                    save_options();
                                  },
                                  false);

  element.rate.addEventListener('change',
                                function() // display rate
                                {
                                  element.rateps.innerHTML =
                                      'x' + (parseFloat(this.value)).toFixed(2);
                                  element.words.innerHTML =
                                      Math.round(this.value * 200);
                                  save_options();
                                },
                                false);
}
...
function restore_options() {
  ...
  element.options_title.innerHTML =
...
function getHotkeys(keys) {
  return keys.substr(0, keys.lastIndexOf('+') + 2) +
         CharCode(keys.substr(keys.lastIndexOf('+') + 2, 2));
}
 ...
 * ---------------------------------------------------------------------------------------------------------------------
 */
function CharCode(code) { return String.fromCharCode(code).toLowerCase(); }
...
function setLocales() {
  ...
  for (i = 0; locales.length; i++) {
    if (locales[i] === undefined)
      break; // Fix 4 Uncaught error
